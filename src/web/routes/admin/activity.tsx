...
function PanelActivityView({
  servers,
  nodes,
  users,
}: {
  servers: ServerItem[] | undefined;
...
function WingsActivityView({
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["admin-wings-activity", page, serverId, nodeId, event],
    queryFn: () =>
      api.get<WingsActivityResponse>(`/activity/wings?${params.toString()}`),
  });

  const hasFilters = serverId || nodeId || event;
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <Tabs defaultValue="panel">
          <TabsList>
            <TabsTrigger value="panel">Panel Activity</TabsTrigger>
            <TabsTrigger value="wings">Wings Activity</TabsTrigger>
          </TabsList>

          <TabsContent value="panel">
            <PanelActivityView nodes={nodes} servers={servers} users={users} />
          </TabsContent>

          <TabsContent value="wings">
            <WingsActivityView nodes={nodes} servers={servers} />
          </TabsContent>
        </Tabs>
      </div>
    </Layout>
