...
function EggsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: { name: string; description: string; dockerImage: string; startup: string; stopCommand: string }) =>
      api.post("/eggs", data),
    onSuccess: () => {
      setCreateOpen(false);
  ...
  const importMutation = useMutation({
    ...
    onError: (err: Error) => setError(err.message === "Unexpected token" ? "Invalid JSON" : err.message),
  });

  const handleCreate = (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    createMutation.mutate({ name, description, dockerImage, startup, stopCommand });
  };
      ...
      <div className="space-y-6">
            ...
            <Dialog open={importOpen} onOpenChange={setImportOpen}>
              ...
              <DialogContent className="max-w-2xl">
                <DialogHeader>
                  ...
                  <DialogDescription>Paste the JSON contents of a Pelican/Pterodactyl egg file, or upload the .json file.</DialogDescription>
                </DialogHeader>
                <form onSubmit={handleImport} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea id="egg-json" value={importJson} onChange={(e) => setImportJson(e.target.value)} placeholder='{"name": "Minecraft", ...}' className="font-mono text-xs h-64" />
                  </div>
                  <DialogFooter>
                    <Button type="submit" disabled={importMutation.isPending || !importJson.trim()}>
            ...
            <Dialog open={createOpen} onOpenChange={setCreateOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>Define a new game server egg configuration.</DialogDescription>
                </DialogHeader>
                <form onSubmit={handleCreate} className="space-y-4">
                  {error && <Alert variant="destructive"><AlertDescription>{error}</AlertDescription></Alert>}
                  <div className="space-y-2"><Label htmlFor="egg-name">Name</Label><Input id="egg-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Minecraft Java" required /></div>
                  <div className="space-y-2"><Label htmlFor="egg-desc">Description</Label><Input id="egg-desc" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Vanilla Minecraft server" /></div>
                  <div className="space-y-2"><Label htmlFor="egg-image">Docker Image</Label><Input id="egg-image" value={dockerImage} onChange={(e) => setDockerImage(e.target.value)} placeholder="ghcr.io/pelican-eggs/games/minecraft:java" required /></div>
                  <div className="space-y-2"><Label htmlFor="egg-startup">Startup Command</Label><Input id="egg-startup" value={startup} onChange={(e) => setStartup(e.target.value)} placeholder="java -Xms128M -Xmx{{SERVER_MEMORY}}M -jar server.jar" required className="font-mono text-sm" /></div>
                  <div className="space-y-2"><Label htmlFor="egg-stop">Stop Command</Label><Input id="egg-stop" value={stopCommand} onChange={(e) => setStopCommand(e.target.value)} placeholder="stop" /></div>
                  <DialogFooter>
                    <Button type="submit" disabled={createMutation.isPending}>{createMutation.isPending ? "Creating..." : "Create Egg"}</Button>
                  </DialogFooter>
                </form>
              </DialogContent>
            </Dialog>
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {eggs.map((egg) => <EggRow key={egg.id} egg={egg} />)}
          </div>
        )}
      </div>
    </Layout>
                    ...
                    <TableBody>
                      {detail.variables.map((v) => (
                        <TableRow key={v.id}>
                          ...
                          <TableCell><Badge variant={v.userEditable ? "default" : "secondary"}>{v.userEditable ? "Yes" : "No"}</Badge></TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
